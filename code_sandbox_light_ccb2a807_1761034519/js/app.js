...
function quickGenerate(style, theme) {
...
function buildPrompt() {
    ...
    if (customPrompt) {
        ...
        prompt = `${customPrompt}, ${styleDesc}`;
    } else {
        // Build automatic prompt
        prompt = `Design graphique plat, ${themeDesc}, ${styleDesc}, composition équilibrée, couleurs vibrantes`;
    }
    
    // Add CRITICAL anti-mockup instruction - UNIVERSAL for all uses
...
async function simulateImageGeneration(prompt) {
    ...
    return {
        imageUrl: `https://via.placeholder.com/800x600/667eea/ffffff?text=${encodeURIComponent('Design ' + state.selectedStyle)}`,
        prompt: prompt,
        style: state.selectedStyle,
        theme: state.selectedTheme,
        ratio: state.selectedRatio,
...
function displayResults(design) {
    ...
    resultsInfo.innerHTML = `
        ...
        <p style="margin-top: 1rem; padding: 0.75rem; background: rgba(102, 126, 234, 0.1); border-left: 3px solid #667eea; border-radius: 4px;">
            <strong>🎯 Utilisation:</strong> Ce design peut être utilisé sur n'importe quel produit (mug, t-shirt, poster, coque, etc.)
        </p>
...
function createResultCard(design) {
    ...
    card.innerHTML = `
        <img src="${design.imageUrl}" alt="Design ${design.style}" class="result-image" />
        <div class="result-info-card">
            <h3>Design ${design.style.charAt(0).toUpperCase() + design.style.slice(1)}</h3>
            <div class="result-specs">
                ...
                <div class="result-spec">
                    ...
                    <span>Thème: ${design.theme}</span>
                </div>
                <div class="result-spec">
                    <i class="fas fa-expand"></i>
                    <span>Format: ${design.ratio}</span>
                </div>
                <div class="result-spec">
                    <i class="fas fa-check-circle"></i>
                    <span>Universel - Tous produits</span>
                </div>
            </div>
            <div class="result-actions">
                <button class="result-btn primary" onclick="downloadDesign('${design.imageUrl}', 'design-${design.style}')">
...
// Load example from localStorage if present (from demo.html)
function loadExampleIfPresent() {
    const exampleStyle = localStorage.getItem('demo_style');
    const exampleTheme = localStorage.getItem('demo_theme');
    const examplePrompt = localStorage.getItem('demo_prompt');
    
    if (exampleStyle && exampleTheme) {
        // Clear from localStorage
        localStorage.removeItem('demo_style');
        localStorage.removeItem('demo_theme');
        localStorage.removeItem('demo_prompt');
        ...
        if (examplePrompt) {
            document.getElementById('customPrompt').value = examplePrompt;
        }
